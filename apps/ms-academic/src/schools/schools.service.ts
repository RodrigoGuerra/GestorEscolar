...
export class SchoolsService {
  ...
  async create(createSchoolDto: CreateSchoolDto): Promise<School> {
    ...
    if (createSchoolDto.isMatrix || isFirstSchool) {
      ...
      await this.schoolsRepository.update({ isMatrix: true }, { isMatrix: false, parentSchoolId: null });
      ...
      await this.schoolsRepository.createQueryBuilder()
        .update(School)
      ...
      const matrix = await this.schoolsRepository.findOne({ where: { isMatrix: true } });
      const school = this.schoolsRepository.create({ 
        ...createSchoolDto, 
  ...
  async update(id: string, updateSchoolDto: UpdateSchoolDto): Promise<School> {
    ...
    if (updateSchoolDto.name || updateSchoolDto.cnpj) {
      const existing = await this.schoolsRepository.createQueryBuilder('school')
  ...
  async remove(id: string): Promise<void> {
    ...
    if (school.isMatrix) {
      const count = await this.schoolsRepository.count();
      if (count > 1) {
        throw new BadRequestException('Não é possível excluir a unidade matriz enquanto houverem filiais. Transforme uma filial em matriz antes da exclusão.');
      }
    }
  ...
  async getSchoolMetrics(schoolId: string) {
    const classesCount = await this.classesRepository.count({ where: { schoolId } });
